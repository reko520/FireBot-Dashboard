...
class ROS2Subscriber(Node):
    ...
    def heatmap_callback(self, msg):
        ...
        shared_data.update(temperature_data=temp_data, sensor_x=new_temp)  # Using temp as dummy distance
...
def create_circular_progress(value):
    ...
    fig.add_trace(go.Pie(
        ...
        hoverinfo='none',
        rotation=90,
        direction='clockwise',  # Always clockwise
        sort=False  
    ))
    fig.update_layout(
        showlegend=False,
...
def ros_thread():
    ...
    time.sleep(1)  # Wait for publishers to initialize
    ...
    with ui.tab_panels(tabs, value='DASHBOARD').classes('w-full'):
        with ui.tab_panel('DASHBOARD'):
            ...
            with ui.grid(columns=3).classes('w-full gap-4 mt-4'):
                ...
                # Events
                with ui.card().classes('w-full h-48'):  # Fixed height card
                    ui.label('EVENTS').classes('text-lg font-medium mb-2')
                    with ui.scroll_area().classes('w-full h-36'):  # Scroll area with slightly less height
...
def update_dashboard():
    with shared_data.lock:
        ...
        if "critical" in shared_data.robot_status.lower():
